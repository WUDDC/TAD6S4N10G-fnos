...
配置界面采用 fnOS 浅色应用风格，左侧导航分为温度、风扇控制、按键控制和硬盘面板；窄屏时导航自动移到顶部，硬盘及按键信息改用卡片布局。
...
三条曲线按 CPU、HDD、NVMe 单列全宽排列，可直接拖动图上的圆点调整；节点带有扩大的鼠标和触控命中区域，并会自动保持温度严格递增、转速不下降，受最低转速和紧急满速温度约束。每条曲线都可增加或删除节点，支持 2–8 个节点；鼠标、触屏和方向键均可操作。HDD 曲线取六个前置 SATA 仓的最高 SMART 温度，NVMe 曲线取四个 M.2 槽位的最高温度，缓存每 60 秒刷新；某类盘没有有效温度时对应曲线不参与控制，也不会为了 2 秒风扇轮询频繁唤醒 SATA 硬盘。升级旧配置时，原硬盘曲线迁移为 HDD 曲线，NVMe 曲线使用上述默认值。
