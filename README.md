...
三条曲线都可直接拖动图上的圆点调整，节点会自动保持温度严格递增、转速不下降，并受最低转速和紧急满速温度约束。每条曲线都可增加或删除节点，支持 2–8 个节点；鼠标、触屏和方向键均可操作。HDD 曲线取六个前置 SATA 仓的最高 SMART 温度，NVMe 曲线取四个 M.2 槽位的最高温度，缓存每 60 秒刷新；某类盘没有有效温度时对应曲线不参与控制，也不会为了 2 秒风扇轮询频繁唤醒 SATA 硬盘。升级旧配置时，原硬盘曲线迁移为 HDD 曲线，NVMe 曲线使用上述默认值。
