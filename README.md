...
风扇功能默认关闭。插件自动检查 IT87 hwmon 节点；未检测到或未加载驱动时，风扇页顶部会显示红色警告和 [fnos-it87-kmod](https://github.com/IamAyang233/fnos-it87-kmod) 安装链接。驱动可用后，插件只列出同时具有 `fanN_input`、`pwmN` 和 `pwmN_enable`，且转速大于 0 RPM 的通道。单风扇机器会自动选中唯一有转速反馈的通道，不会写入未接线的风扇接口。
