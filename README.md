...
界面中的功耗设置、风扇控制和按键映射分别独立保存；保存当前页面不会覆盖另外页面尚未提交的调整。侧栏的“检查更新”只在用户点击后请求最新 Release，不会自动联网检查。

配置界面采用 fnOS 原生浅色单层布局，侧栏提供监控面板、风扇控制、按键映射、功耗设置、调试和关于六个页面，并整合版本、更新检查与健康状态；侧栏和内容区独立滚动，硬盘机箱素材会随窗口宽度缩放并保持同排显示。
