...
界面中的全局配置、风扇控制和按钮控制分别独立保存；保存当前页面不会覆盖另外两个页面尚未提交的调整。标题旁的“GitHub · 检查更新”只在用户点击后请求最新 Release，不会自动联网检查。

配置界面采用 fnOS 浅色应用风格，左侧导航分为全局配置、风扇控制、按键控制和硬盘面板；窄屏时导航自动移到顶部，硬盘及按键信息改用卡片布局。
...
风扇功能默认关闭。插件自动检查 IT87 hwmon 节点；未检测到或未加载驱动时，风扇页顶部会显示红色警告和 [fnos-it87-kmod](https://github.com/IamAyang233/fnos-it87-kmod) 安装链接。驱动已检测但没有完整 PWM/RPM 通道，或通道转速均为 0 RPM 时，也会给出对应的接线/适配提示。驱动可用后，插件只列出同时具有 `fanN_input`、`pwmN` 和 `pwmN_enable`，且转速大于 0 RPM 的通道。单风扇机器会自动选中唯一有转速反馈的通道，不会写入未接线的风扇接口。
...
每次打开插件界面时，插件都会在首次硬盘刷新完成后检查 CPU `coretemp`、IT87 驱动、风扇 PWM/RPM 通道、硬盘仓位枚举及已识别硬盘的 SMART/NVMe 温度。发现异常会在本次打开中自动弹出一次针对性说明，并持续保留在右上角“需要检查”提示中；关闭提示后，当前界面的 5 秒状态轮询不会反复打扰，但下次重新打开插件仍会重新检查并弹出。提示只代表插件当前没有读到相应标准接口；空仓不会被当成温度故障，具体问题仍需结合驱动、接线和槽位映射判断。
...
温度页用十个紧凑槽位卡片显示 SATA 6 → 1 与 NVMe 1 → 4 的温度、插入状态和活动状态；硬盘面板的文字表格继续显示 Linux 设备名、型号、序列号、容量、阵列/挂载用途、SMART 健康与温度。活动状态分为繁忙、空闲、休眠和未知：繁忙表示内核当前存在在途 I/O，或两次刷新之间读写计数发生变化；SATA 休眠来自 `smartctl -n standby` 返回的电源模式。后台每 60 秒刷新一次，普通刷新不会为了读取健康或电源状态强制唤醒休眠盘；按键动作中的“SMART 检查”会执行一次主动检查。仓位映射仅适用于该主板和背板，不能按设备名 `sda/sdb` 猜测物理位置。
