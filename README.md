...
这是一个面向飞牛 fnOS 与 TAD6S4N10G 的第三方模块。它按 RR CPUinfo 的口径显示 Intel Processor N100 与 Intel Core i3-N305 的 CPU 核心最高温度，同时保留 Package 原始温度供核对；还可限制 Package 功耗、按 CPU/硬盘双温度曲线控制单风扇、显示硬盘物理仓位，并配置机箱 GPIO 按键映射。
...
默认 CPU 曲线为 40°C/60%、55°C/70%、70°C/85%、80°C/100%；默认硬盘曲线为 25°C/最低转速、35°C/85%、50°C/100%。两条曲线分别计算目标 PWM，插件始终使用较高值，因此 CPU 或硬盘任一方先升温都会提高风扇转速。紧急满速温度为 85°C，风扇采样间隔为 2 秒。CPU 控制温度取所有 `coretemp` 节点中的最高值，不读取无标签的 IT87 温度，也不会把 CPU 与 GPU 温度相加。

CPU 与硬盘曲线都可直接拖动图上的圆点调整，节点会自动保持温度严格递增、转速不下降，并受最低转速和紧急满速温度约束。每条曲线都可增加或删除节点，支持 2–8 个节点；鼠标、触屏和方向键均可操作。硬盘温度取现有仓位 SMART 缓存的最高值，缓存每 60 秒刷新；没有有效盘温时硬盘曲线不参与控制，也不会为了 2 秒风扇轮询频繁唤醒 SATA 硬盘。
