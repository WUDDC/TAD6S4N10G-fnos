...
界面中的全局配置、风扇控制和按钮控制分别独立保存；保存当前页面不会覆盖另外两个页面尚未提交的调整。
...
每次打开新版本界面时，插件会在首次硬盘刷新完成后检查 CPU `coretemp`、IT87 驱动、风扇 PWM/RPM 通道、硬盘仓位枚举及已识别硬盘的 SMART/NVMe 温度。发现异常会自动弹出一次针对性说明，并持续保留在右上角“需要检查”提示中。提示只代表插件当前没有读到相应标准接口；空仓不会被当成温度故障，具体问题仍需结合驱动、接线和槽位映射判断。
